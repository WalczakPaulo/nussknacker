...
class PartSubGraphCompiler(protected val classLoader: ClassLoader,
  ...
  private def compileSubsequent(ctx: ValidationContext, data: OneOutputSubsequentNodeData, next: Next)(implicit nodeId: NodeId): CompilationResult[Node] = data match {
    ...
    case SubprocessInput(id, ref, _, _, subParams) =>
      ...
      val getSubprocessParam = (paramName: String) => {
        val subParam = subParams.get.find(_.name == paramName).get
        val typingR = subParam.typ.toTyped(classLoader).getOrElse(throw new IllegalArgumentException("Failed to load subprocess parameter type"))
        Parameter(paramName, typingR, typingR)
      }

      val validParams = expressionCompiler.compileObjectParameters(
        ref.parameters.map((p: evaluatedparam.Parameter) => getSubprocessParam(p.name)),
        ref.parameters,
        toOption(ctx))
